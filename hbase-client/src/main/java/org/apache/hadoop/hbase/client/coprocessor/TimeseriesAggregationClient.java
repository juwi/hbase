 ...
 * Region Server side via the TimeseriesAggregateService. This class will
 * implement the supporting functionality for summing/processing the individual
 * results obtained from the TimeseriesAggregateService for each region.
 * <p>
 ...
 * 
 * <T> Cell value data type <S> Promoted data type
 * <P>
 * PB message that is used to transport initializer specific bytes
 * <Q>PB message that is used to transport Cell (<T>) instance <R> PB message
 * that is used to transport Promoted (<S>) instance
 */
@InterfaceAudience.Private
public class TimeseriesAggregationClient {

	private static final Log log = LogFactory
			.getLog(TimeseriesAggregationClient.class);
	Configuration conf;
							...
							if (max.containsKey(entry.getKey())) {
								...
								max.put(entry.getKey(),
										(current == null || (candidate != null && ci
												.compare(current, candidate) < 0)) ? candidate
												: current);
							} else {
								max.put(entry.getKey(),
	 ...
	 * @throws Throwable
	 */
	private <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, Pair<List<S>, Long>> getStdArgs(
			final Table table, final ColumnInterpreter<R, S, P, Q, T> ci,
			final Scan scan) throws Throwable {
		final TimeseriesAggregateRequest requestArg = validateArgAndGetPB(scan,
				ci, false);
		class StdCallback implements
				Batch.Callback<TimeseriesAggregateResponse> {
				...
				for (TimeseriesAggregateResponseMapEntry entry : results) {
					if (entry.getValue().getFirstPartCount() == 0) {
						if (!stds.containsKey(entry.getKey())) {
							stds.put(entry.getKey(), new Pair<List<S>, Long>(
									null, 0L));
						}
					} else {
						...
						if (stds.containsKey(entry.getKey())) {
							List<S> currentList = stds.get(entry.getKey())
									.getFirst();

							S sumVal = ci.add(currentList.get(0), list.get(0));
							S sumSqVal = ci
									.add(currentList.get(1), list.get(1));
							long count = bb.getLong()
									+ stds.get(entry.getKey()).getSecond();

							stds.put(entry.getKey(), new Pair<List<S>, Long>(
									Arrays.asList(sumVal, sumSqVal), count));
						} else {
							stds.put(entry.getKey(), new Pair<List<S>, Long>(
									list, bb.getLong()));
						}
	 ...
	 * @throws Throwable
	 */
	public <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, Double> std(
	...
	public <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, Double> std(
			...
			throws Throwable {
		ConcurrentSkipListMap<Long, Pair<List<S>, Long>> p = getStdArgs(table,
				ci, scan);
		...
		for (Map.Entry<Long, Pair<List<S>, Long>> entry : p.entrySet()) {
			double res = 0d;
			double avg = ci.divideForAvg(entry.getValue().getFirst().get(0),
					entry.getValue().getSecond());
			double avgOfSumSq = ci.divideForAvg(entry.getValue().getFirst()
					.get(1), entry.getValue().getSecond());
	 ...
	 * @throws Throwable
	 */
	private <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, Pair<NavigableMap<byte[], List<S>>, List<S>>> getMedianArgs(
			final Table table, final ColumnInterpreter<R, S, P, Q, T> ci,
			final Scan scan) throws Throwable {
		final TimeseriesAggregateRequest requestArg = validateArgAndGetPB(scan,
				ci, false);
		class StdCallback implements
				Batch.Callback<TimeseriesAggregateResponse> {
			ConcurrentSkipListMap<Long, Pair<NavigableMap<byte[], List<S>>, List<S>>> medians = new ConcurrentSkipListMap<>();

			public synchronized ConcurrentSkipListMap<Long, Pair<NavigableMap<byte[], List<S>>, List<S>>> getMedianParams() {
				return medians;
			}
				...
				for (TimeseriesAggregateResponseMapEntry entry : results) {
					if (entry.getValue().getFirstPartCount() == 0) {
						if (!medians.containsKey(entry.getKey())) {
							medians.put(
									entry.getKey(),
									new Pair<NavigableMap<byte[], List<S>>, List<S>>(
											new TreeMap<byte[], List<S>>(),
											new ArrayList<S>()));
						}
					} else {
						List<S> list = new ArrayList<S>();
						...
						NavigableMap<byte[], List<S>> map = new TreeMap<byte[], List<S>>(
								Bytes.BYTES_COMPARATOR);

						if (medians.containsKey(entry.getKey())) {
							List<S> values = list;
							values.addAll(medians.get(entry.getKey())
									.getSecond());
							map.put(row, values);

							Pair<NavigableMap<byte[], List<S>>, List<S>> current = medians
									.get(entry.getKey());
							S sumVal = ci.add(current.getSecond().get(0),
									list.get(0));
							S sumWeights = ci.add(current.getSecond().get(1),
									list.get(1));
	 ...
	 * @throws Throwable
	 */
	public <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, R> median(
			final TableName tableName, ColumnInterpreter<R, S, P, Q, T> ci,
			Scan scan) throws Throwable {
		Table table = null;
		try {
	...
	public <R, S, P extends Message, Q extends Message, T extends Message> ConcurrentSkipListMap<Long, R> median(
		...
		ConcurrentSkipListMap<Long, Pair<NavigableMap<byte[], List<S>>, List<S>>> p = getMedianArgs(
				table, ci, scan);
		byte[] startRow = null;
		byte[] colFamily = scan.getFamilies()[0];
		NavigableSet<byte[]> quals = scan.getFamilyMap().get(colFamily);
		for (Map.Entry<Long, Pair<NavigableMap<byte[], List<S>>, List<S>>> entry : p
				.entrySet()) {
			...
			for (Map.Entry<byte[], List<S>> innerMapEntry : map.entrySet()) {
				S s = weighted ? innerMapEntry.getValue().get(1)
						: innerMapEntry.getValue().get(0);
			...
			try {
				...
				byte[] weightQualifier = weighted ? quals.pollLast()
						: qualifier;
				R value = null;
				do {
					...
					if (results != null && results.length > 0) {
						for (int i = 0; i < results.length; i++) {
							Result r = results[i];
							// retrieve weight
							Cell kv = r.getColumnLatestCell(colFamily,
									weightQualifier);
							R newValue = ci.getValue(colFamily,
									weightQualifier, kv);
	...
	<R, S, P extends Message, Q extends Message, T extends Message> TimeseriesAggregateRequest validateArgAndGetPB(
		...
		final TimeseriesAggregateRequest.Builder requestBuilder = TimeseriesAggregateRequest
				.newBuilder();
		requestBuilder
