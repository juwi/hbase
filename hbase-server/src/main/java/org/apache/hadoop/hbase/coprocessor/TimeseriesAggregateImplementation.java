...
public class TimeseriesAggregateImplementation<T, S, P extends Message, Q extends Message, R extends Message>
	...
	public void getMax(RpcController controller,
		...
		try {
			...
			if (hasScannerRange) {
				intervalRange = new TimeRange(scan.getTimeRange().getMin(),
						scan.getTimeRange().getMin()
								+ request.getTimeIntervalSeconds() * 1000);
			} else {
				intervalRange = new TimeRange(request.getRange()
						.getKeyTimestampMin() * 1000,
						(request.getRange().getKeyTimestampMin() + request
								.getTimeIntervalSeconds()) * 1000);
			}
			...
			do {
				...
				for (Cell kv : results) {
					...
					if (hasScannerRange)
						...
						timestamp = Bytes.toLong(kv.getRowArray(),
								kv.getRowOffset(), kv.getRowLength()) * 1000;
					if (!intervalRange.withinTimeRange(timestamp)) {
						intervalRange = new TimeRange(intervalRange.getMax(),
								intervalRange.getMax()
	...
	ColumnInterpreter<T, S, P, Q, R> constructColumnInterpreterFromRequest(
		...
		try {
			...
			ColumnInterpreter<T, S, P, Q, R> ci = (ColumnInterpreter<T, S, P, Q, R>) cls
					.newInstance();
			if (request.hasInterpreterSpecificBytes()) {
				ByteString b = request.getInterpreterSpecificBytes();
				P initMsg = ProtobufUtil.getParsedGenericInstance(
						ci.getClass(), 2, b);
		...
		try {
			...
			if (!sums.isEmpty()) {
				...
				for (Map.Entry<TimeRange, S> entry : sums.entrySet()) {
					...
					valueBuilder.addFirstPart(ci.getProtoForPromotedType(
							entry.getValue()).toByteString());
	...
	public void getAvg(RpcController controller,
		...
		try {
			...
			do {
				...
				for (Cell kv : results) {
					...
					if (intervalRange.withinTimeRange(timestamp)) {
						...
						if (temp != null)
							sumVal = ci.add(sumVal, ci.castToReturnType(temp));
						averages.put(new TimeRange(intervalRange.getMin(),
								intervalRange.getMax()),
								new AbstractMap.SimpleEntry<Long, S>(
										kvCountVal, sumVal));
					}
				}
			} while (hasMoreRows);
			if (!averages.isEmpty()) {
				...
				for (Entry<TimeRange, SimpleEntry<Long, S>> entry : averages
						.entrySet()) {
					TimeseriesAggregateResponseEntry responseElement = null;
					TimeseriesAggregateResponseMapEntry mapEntry = null;
					TimeseriesAggregateResponseEntry.Builder valueBuilder = TimeseriesAggregateResponseEntry
					...
					ByteString first = ci.getProtoForPromotedType(
							entry.getValue().getValue()).toByteString();
					valueBuilder.addFirstPart(first);
					ByteBuffer bb = ByteBuffer.allocate(8).putLong(
	...
	public void getStd(RpcController controller,
		...
		TimeRange intervalRange = null;
		InternalScanner scanner = null;
		Map<TimeRange, SimpleEntry<Long, Pair<S, S>>> stds = new HashMap<TimeRange, SimpleEntry<Long, Pair<S, S>>>();
		boolean hasScannerRange = false;
		...
		try {
			...
			do {
				...
				for (Cell kv : results) {
					...
					if (intervalRange.withinTimeRange(timestamp)) {
						kvCountVal++;
						tempVal = ci.add(tempVal, ci.castToReturnType(ci
								.getValue(colFamily, qualifier, kv)));
						sumVal = ci.add(sumVal, tempVal);
						sumSqVal = ci.add(sumSqVal,
								ci.multiply(tempVal, tempVal));
						stds.put(new TimeRange(intervalRange.getMin(),
								intervalRange.getMax()),
								new AbstractMap.SimpleEntry<Long, Pair<S, S>>(
										kvCountVal, new Pair<S, S>(sumVal,
												sumSqVal)));
					}
				}
			} while (hasMoreRows);
			if (!stds.isEmpty()) {
				...
				for (Entry<TimeRange, SimpleEntry<Long, Pair<S, S>>> entry : stds
						.entrySet()) {
					TimeseriesAggregateResponseMapEntry.Builder mapEntryBuilder = TimeseriesAggregateResponseMapEntry
							.newBuilder();

					ByteString first_sumVal = ci.getProtoForPromotedType(
							entry.getValue().getValue().getFirst())
							.toByteString();
					ByteString first_sumSqVal = ci.getProtoForPromotedType(
							entry.getValue().getValue().getSecond())
							.toByteString();
					TimeseriesAggregateResponseEntry.Builder responsePair = TimeseriesAggregateResponseEntry
							.newBuilder();
		...
		try {
			...
			do {
				...
				for (Cell kv : results) {
					...
					if (intervalRange.withinTimeRange(timestamp)) {
						tempVal = ci.add(tempVal, ci.castToReturnType(ci
								.getValue(colFamily, valQualifier, kv)));
						if (weightQualifier != null) {
							tempWeight = ci.add(tempWeight, ci
									.castToReturnType(ci.getValue(colFamily,
											weightQualifier, kv)));
						}
						sumVal = ci.add(sumVal, tempVal);
						sumWeights = ci.add(sumWeights, tempWeight);
						medians.put(new TimeRange(intervalRange.getMin(),
								intervalRange.getMax()), new Pair<S, S>(sumVal,
								tempVal));
					}
				}
			} while (hasMoreRows);
			if (!medians.isEmpty()) {
				...
				for (Entry<TimeRange, Pair<S, S>> entry : medians.entrySet()) {
					...
					ByteString first_sumVal = ci.getProtoForPromotedType(
							entry.getValue().getFirst()).toByteString();
					S s = entry.getValue().getSecond() == null ? ci
							.castToReturnType(ci.getMinValue()) : entry
							.getValue().getSecond();
